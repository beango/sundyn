...
public class CyclicBarrierExecutorImpl<T, R> implements ListTaskExecutor<T, R> {
    ...
    @Override
    public List<R> execute(List<T> orignList, final ListTaskCallable<T, R> callable) throws ListTaskException {
        final List<R> resultList = new CopyOnWriteArrayList<>();
        final CountDownLatch countDownLatch = new CountDownLatch(orignList.size());
        for (T t : orignList) {
            final List<T> subOriginList = new ArrayList<>();
            subOriginList.add(t);
