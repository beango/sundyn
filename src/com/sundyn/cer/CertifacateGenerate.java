...
public class CertifacateGenerate {
    ...
    public void GenRootKeyPair() throws Exception {
        ...
        writeObject(rootpath + "/cer/zxroot.public", keyPair.getPublic());
        writeObject(rootpath + "/cer/zxroot.private", keyPair.getPrivate());
    }
     ...
     */
    @Test
    public void ZhangsanKeyPair(String publicStorePath, String privateStorePath) throws Exception {
        KeyPairGenerator keyPairGenerator = KeyPairGenerator.getInstance(KEY_PAIR_ALG,"BC");
        keyPairGenerator.initialize(2048);
        KeyPair keyPair = keyPairGenerator.generateKeyPair();
        writeObject(publicStorePath, keyPair.getPublic());
        writeObject(privateStorePath, keyPair.getPrivate());
    }
     ...
     */
    @Test
    public void GenZhangsanCert(String cerpath, Date expTime, String userCerPub, String dnUser) throws Exception {
        X509V3CertificateGenerator certGen = new X509V3CertificateGenerator();
        certGen.setIssuerDN(new X500Principal(DN_CA));
        certGen.setNotAfter(expTime);
        certGen.setNotBefore(new Date());
        certGen.setPublicKey(getZhangsanPublicKey(userCerPub));
        certGen.setSerialNumber(BigInteger.TEN);
        certGen.setSignatureAlgorithm(SIG_ALG);
        certGen.setSubjectDN(new X500Principal(dnUser));
        X509Certificate certificate = certGen.generate(getRootPrivateKey(),"BC");
