...
public class AnalyseService extends SuperDao
{
    public List AnalyseTotal(final String startDate, final String endDate, final String allId, final String analyseType, final String ids) {
        String sql = "select count(*) as num, left(JieshouTime,10) as serviceDate from appries_appries where JieshouTime >='" + startDate + "' and JieshouTime<='" + endDate + "'  and  keyno in (" + allId + ")  and deptId in(" + ids + ") ";
        if (analyseType.equals("day")) {
            sql = String.valueOf(sql) + " group by left(JieshouTime,10) ";
        }
        else if (analyseType.equals("month")) {
            sql = String.valueOf(sql) + " group by left(JieshouTime,7)";
        }
