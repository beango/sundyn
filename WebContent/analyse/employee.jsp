...
<head>
    ...
    <script language="javascript">
        $(document).ready(function () {
            initTree("?depttype=3&isOnlyLeaf=0", '<%=request.getParameter("deptId")%>');
        });
    </script>
    <style type="text/css">
        ul.ztree {
...
<div class="layui-inline">
    <label class="layui-form-label" style="width:45px;"><s:text name='sundyn.analyse.by'/></label>
    <div class="layui-input-inline">
...
<script type="text/javascript">
    ...
    function loadChart(id){
        ...
        if(id == '#tab1'){
            analyseEmployeeAjax(w, "servicecount", 1, lay);
        }
        if(id == '#tab2'){
            analyseEmployeeAjax(w, "cancelcount", 2, lay);
        }
        if(id == '#tab3'){
            analyseEmployeeAjax(w, "totalwaittime", 3, lay);
        }
        if(id == '#tab4'){
            analyseEmployeeAjax(w, "totalservicetime", 4, lay);
        }
        if(id == '#tab5'){
            analyseEmployeeAjax(w, "totalkeymy", 5, lay);
        }
        if(id == '#tab6'){
            analyseEmployeeAjax(w, "totalmyd", 6, lay);
        }
    }
