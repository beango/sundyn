...
dTree.prototype.node = function(node, nodeId) {
	...
	if (this.config.useIcons) {
		...
		if(this.config.check==true){
			str+= '<input type="checkbox" id="check'+ this.obj + nodeId + '"  value="'+ node.id +'" onclick="javascript:'+this.obj+'.dTreeCheckbox('+nodeId+')"/>';   
		}
...
dTree.prototype.dTreeCheckbox=function(nodeId){
	...
	if(selectType==false){
		...
		do{
			for(j=0;j<len;j++){
				if(this.aNodes[j].pid==clicknode.pid&&document.getElementById("check"+this.obj+j).checked==true){
					return;
				}
			}
			if(j==len){
				for(k=0;k<len;k++){
					if(this.aNodes[k].id==clicknode.pid){
						document.getElementById("check"+this.obj+k).checked=false;
