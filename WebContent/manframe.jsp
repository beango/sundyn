<%@page pageEncoding="UTF-8"%>
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		<meta http-equiv="Content-Type" content="text/html; charset=gb2312" />
		<title>无标题文档</title>
		<link rel="stylesheet" href="css/common_<s:text name='sundyn.language' />.css" type="text/css"></link>
	</head>
	<body>
		<div id="man_zone">
			<div class="fengge">
				&nbsp;
			</div>
			<div class="neirong">
				<div class="neirong_lanmu" style="float: left;">
					<div class="neirong_lanmu_top">
						<span class="px13">评价信息流水</span>
						<span style="padding-left: 180px;">详细>></span>
					</div>
					<table width="100%" height="175" cellpadding="0" cellspacing="0">
						<tr>
							<td align="center" valign="middle"
								background="images/table_bg_03.jpg" class="px13_1">
								时间
							</td>
							<td align="center" valign="middle"
								background="images/table_bg_03.jpg" class="px13_1">
								卡号
							</td>
							<td align="center" valign="middle"
								background="images/table_bg_03.jpg" class="px13_1">
								姓名
							</td>
							<td align="center" valign="middle"
								background="images/table_bg_03.jpg" class="px13_1">
								评价
							</td>
						</tr>
						<tr>
							<td align="center" valign="middle">
								2009-12-20 17:58
							</td>
							<td align="center" valign="middle">
								100011
							</td>
							<td align="center" valign="middle">
								张晓月
							</td>
							<td align="center" valign="middle">
								满意
							</td>
						</tr>
						<tr>
							<td align="center" valign="middle">
								2009-12-20 17:58
							</td>
							<td align="center" valign="middle">
								100011
							</td>
							<td align="center" valign="middle">
								张晓月
							</td>
							<td align="center" valign="middle">
								满意
							</td>
						</tr>
						<tr>
							<td align="center" valign="middle">
								2009-12-20 17:58
							</td>
							<td align="center" valign="middle">
								100011
							</td>
							<td align="center" valign="middle">
								张晓月
							</td>
							<td align="center" valign="middle">
								满意
							</td>
						</tr>
						<tr>
							<td align="center" valign="middle">
								2009-12-20 17:58
							</td>
							<td align="center" valign="middle">
								100011
							</td>
							<td align="center" valign="middle">
								张晓月
							</td>
							<td align="center" valign="middle">
								满意
							</td>
						</tr>
						<tr>
							<td align="center" valign="middle">
								2009-12-20 17:58
							</td>
							<td align="center" valign="middle">
								100011
							</td>
							<td align="center" valign="middle">
								张晓月
							</td>
							<td align="center" valign="middle">
								满意
							</td>
						</tr>
					</table>

				</div>
				<div class="neirong_lanmu" style="float: right;">
					<div class="neirong_lanmu_top">
						<span class="px13">不满意评价信息</span>
						<span style="padding-left: 180px;">详细>></span>
					</div>
					<table width="100%" height="175" cellpadding="0" cellspacing="0">
						<tr>
							<td align="center" valign="middle"
								background="images/table_bg_03.jpg" class="px13_1">
								时间
							</td>
							<td align="center" valign="middle"
								background="images/table_bg_03.jpg" class="px13_1">
								卡号
							</td>
							<td align="center" valign="middle"
								background="images/table_bg_03.jpg" class="px13_1">
								姓名
							</td>
							<td align="center" valign="middle"
								background="images/table_bg_03.jpg" class="px13_1">
								评价
							</td>
						</tr>
						<tr>
							<td align="center" valign="middle">
								2009-12-20 17:58
							</td>
							<td align="center" valign="middle">
								100011
							</td>
							<td align="center" valign="middle">
								张晓月
							</td>
							<td align="center" valign="middle">
								满意
							</td>
						</tr>
						<tr>
							<td align="center" valign="middle">
								2009-12-20 17:58
							</td>
							<td align="center" valign="middle">
								100011
							</td>
							<td align="center" valign="middle">
								张晓月
							</td>
							<td align="center" valign="middle">
								满意
							</td>
						</tr>
						<tr>
							<td align="center" valign="middle">
								2009-12-20 17:58
							</td>
							<td align="center" valign="middle">
								100011
							</td>
							<td align="center" valign="middle">
								张晓月
							</td>
							<td align="center" valign="middle">
								满意
							</td>
						</tr>
						<tr>
							<td align="center" valign="middle">
								2009-12-20 17:58
							</td>
							<td align="center" valign="middle">
								100011
							</td>
							<td align="center" valign="middle">
								张晓月
							</td>
							<td align="center" valign="middle">
								满意
							</td>
						</tr>
						<tr>
							<td align="center" valign="middle">
								2009-12-20 17:58
							</td>
							<td align="center" valign="middle">
								100011
							</td>
							<td align="center" valign="middle">
								张晓月
							</td>
							<td align="center" valign="middle">
								满意
							</td>
						</tr>
					</table>

				</div>
				<div class="fengge">
					&nbsp;
				</div>
				<div class="neirong_lanmu" style="float: left;">
					<div class="neirong_lanmu_top">
						<span class="px13">满意率曲线图</span>
						<span style="padding-left: 180px;">详细>></span>
					</div>
					<div style="height: 175px;" class="kuang"></div>
				</div>
				<div class="neirong_lanmu" style="float: right;">
					<div class="neirong_lanmu_top">
						<span class="px13">不满意比重</span>
						<span style="padding-left: 200px;"> 详细>></span>
					</div>
					<div style="height: 175px;" class="kuang"></div>
				</div>
			</div>
		</div>
		<div style="float: left;"><img src="images/man_zone_bottom.gif" width="763" height="7" border="0" /></div>
	</body>
</html>
